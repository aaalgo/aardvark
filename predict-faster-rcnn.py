...
class Model:
    def __init__ (self, X, anchor_th, nms_max, nms_th, path, name):
        ...
        self.predictions = tf.import_graph_def(mg.graph_def, name=name,
                    ...
                    return_elements=['rpn_boxes:0', 'rpn_probs:0', 'boxes:0', 'rpn_all_probs:0'])
...
flags.DEFINE_integer('min_size', 200, '')
flags.DEFINE_float('vis_downsize', 1, '')

def save_prediction_image (path, image, preds):
    image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
    rpn_boxes, _, boxes, ap = preds
    
    if FLAGS.vis_downsize != 1:
        image = cv2.resize(image, None, fx=1.0/FLAGS.vis_downsize, fy=1.0/FLAGS.vis_downsize)
    rpn_boxes = np.round(rpn_boxes / FLAGS.vis_downsize).astype(np.int32)
    ...
    '''
    H, W, _ = image.shape
    ap = np.reshape(ap, (H//4, W//4))
    ap = cv2.cvtColor(cv2.resize(ap * 255, (W, H)), cv2.COLOR_GRAY2BGR)
    
    image = np.concatenate((image, ap), axis=1) 
    '''
